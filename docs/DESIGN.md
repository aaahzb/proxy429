...
- **(removed keys)**: the old top-level `classifier_thinking_disabled` / `classifier_max_tokens` / `translateNone2Low` were removed — `classifier_thinking` moved into `classifier_route`, and `translateNone2Low` became the per-route `convertOff2Low` (see below). A config still carrying any of the three fails to load with a migration hint naming the replacement (the web console's save probes first and refuses to write).
