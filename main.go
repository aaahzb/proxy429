...
type Config struct {
	...
	TranslateNone2Low          bool             `json:"translateNone2Low"`             // Responses 翻译口：①下游关思考的请求悄悄升级为 low 思考发上游、回传剥离思考块（下游无感知，usage 如实透传）；②工具续轮历史不可回放时代理本要自行关思考——关会触发 Kimi 把 K3 路由 K2.8 无思考版，开关开着则按下游所请档位发（没给/不认识 → low 保底；不剥思考块，带回签名让下轮历史自愈；上游拒则 400 兜底回退关思考重试）；默认 false。动机：Kimi 文档「关闭 thinking 后路由到 K2.8 Preview 无思考版」，开着思考避免 K3 被降级路由
...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	// 按下游所请档位发，见 n2l 常量）：API 列对隐式升级显双色徽标 [off->low]；转发循环里
	...
	// 看到的是升级后的 low，体现不出「下游关、上游开」的悄悄升级语义）。历史兜底开思考
	//（按下游所请档位发）不覆盖：如实显示所请档位即可（下游本来就要思考）。
	...
	for attempt := 0; attempt <= c.MaxRetries; attempt++ {
		...
		if headHasError(head) {
			...
			// translateNone2Low 升级被上游拒（如无思考历史却开思考——历史兜底开思考主要
			// 防这个；拒与档位无关，按所请档位发、被拒再退）：把 thinking 改回关闭立即
			// 重试一次——下游本来要的就是关思考（或
