...
看到 `代理启动: 监听 127.0.0.1:8080 -> 转发到 https://ark.cn-beijing.volces.com/api/plan` 就说明跑起来了。
...
```
[流式] 活跃1 | 流出 2.3KB (12KB/s) | 缓存命中 0 | 缓存写入 0 | 输入 24 | 输出 80
```

- **活跃**：当前正在透传的流数量。
- **流出 / 速率**：累计已转发字节 + 最近 100ms 的字节速率。每转发一段 SSE 就涨，**流过程中实时跳动**，是「正在迸出」的直接体感。
- **缓存命中 / 缓存写入 / 输入 / 输出**：从 SSE 的 `usage` 解析的累计 token。
